...
from tests.conftest import TREASURY_ADDRESS
...
def unfunded_merkle_drop_contract(
    ...
    treasury_address = web3.eth.accounts[0]
    airdrop_expires_at = pendulum.now().int_timestamp + 10
...
def test_status_cli_success(runner, funded_merkle_drop_contract, token_contract):
    ...
    assert (
        funded_merkle_drop_contract.functions.treasuryAddress().call() in result.output
    )
    assert (
        str(funded_merkle_drop_contract.functions.airdropExpiresAt().call())
        in result.output
    )
